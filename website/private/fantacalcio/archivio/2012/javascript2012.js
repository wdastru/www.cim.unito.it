	...
	if( document.getElementById ) 
	{
		...
		if ( visibility == 'hidden' )
		{
			...
			// i primi tre input sono send close e reset.
 			inputTags.item(1).value = retiA = trimWS(goals_casa).replace(/\&nbsp\;/g,'');			
			inputTags.item(2).value = retiB = trimWS(goals_fuori).replace(/\&nbsp\;/g,''); // trim white spaces			
			...
			if ( isNaN(goalsTot) ) 
 			...
 			inputTags.item(3).value = trimWS(punti_casa).replace(/\&nbsp\;/g,'');			
			inputTags.item(4).value = trimWS(punti_fuori).replace(/\&nbsp\;/g,''); // trim white spaces			
...
{	
	if( document.getElementById ) 
	{
		...
		var repeatPasswordFieldOffer = document.getElementById('repeatPasswordFieldOffer');
		...
		if (passwordFieldTeam.value == '' || passwordFieldOffer.value == '' || repeatPasswordFieldOffer.value == '' || trimWS(bidField.value) == '' || isNaN(trimWS(bidField.value)) || trimWS(playerOutField.value) == '' || trimWS(roleOutField.value) == '' || trimWS(teamOutField.value) == '') // controlla che password non sia vuoto e che gli altri campi non contengano solo spazi bianchi e che bidField sia un numero
...
function enablePasswordButton()
{
	if( document.getElementById ) 
	{
		...
		if (passwordField.value == '') // controlla che il campo password non sia vuoto
...
function PopupCentrata(what, w, h) 
{
	...
	window.open(what,"","width=" + w + ",height=" + h + ",top=" + t + ",left=" + l + ",scrollbars=1");
}
...
function toggleAvulse()
{
	if( document.getElementById ) 
	{
		if (document.getElementById('avulse').style.display == 'none' || document.getElementById('avulse').style.display == '')
...
function showAvulseToggleButton()
{
	if( document.getElementById ) 
	{
		if (document.getElementById('avulse_wrapper').style.display == 'none' || document.getElementById('avulse').style.display == '')
