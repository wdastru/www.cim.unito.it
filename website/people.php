<?php $localizer = './'; ?>
<!DOCTYPE html>
<html lang="it">
    <!-- InstanceBegin template='/Templates/home.dwt' codeOutsideHTMLIsLocked='false' -->
    <head>
        <?php require $localizer . 'includes/head_const.inc.php'?>
        <title>Molecular Imaging Center - University of Torino - People</title>
        <link rel="stylesheet" href="square.css" />
        <meta
            name="description"
            content="The people that works at the Molecular Imaging Center"
        />
    </head>
    <body>
        <div id="section8">
            <div id="subsection0">
                <?php include 'includes/menu.php'; ?> <?php include
                'includes/header.php'; ?>
                <!-- InstanceEndEditable -->
                <div id="content-nosidebar">
                    <div class="paddingOuter">
                        <div class="band">
                            <h1
                                class="subsectionTitle"
                                id="staff_institution_1"
                            >
                                <!-- InstanceBeginEditable name='subsection title' -->
                                University of Torino
                                <!-- InstanceEndEditable -->
                            </h1>
                        </div>
                        <div class="paddingInner">
                            <h1>
                                <span id="staff_title_1"
                                    >Research Group Leaders</span
                                >
                            </h1>
                            <ul class="staff">
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/aime.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Homepage"
                                            href="aboutus.php#director"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Silvio Aime</b>
                                                Director</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/terreno.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Homepage"
                                            href="PI/Terreno/home.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Enzo Terreno</b> Associate
                                                Professor</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/geninatti_crich.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Homepage"
                                            href="PI/Geninatti/home.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Simonetta Geninatti</b>
                                                Associate Professor</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/gianolio.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Homepage"
                                            href="PI/Gianolio/home.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Eliana Gianolio</b>
                                                Associate Professor</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/dastru.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Homepage"
                                            href="#"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Walter Dastr&ugrave;</b>
                                                Researcher</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/reineri.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Homepage"
                                            href="PI/Reineri/home.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Francesca Reineri</b>
                                                Associate Professor</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/dellicastelli.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Homepage"
                                            href="PI/Delli_Castelli/home.php"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Daniela Delli Castelli</b>
                                                Assistant Professor</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/viale.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Homepage"
                                            href="#"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Alessandra Viale</b>
                                                Researcher</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/cutrin.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Homepage"
                                            href="PI/Cutrin/home.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Juan Carlos Cutrin</b>
                                                Assistant Professor</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/ferrauto.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_ferrauto.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Giuseppe Ferrauto</b>
                                                Research Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                            </ul>
                            <h1>
                                <span id="staff_title_2"
                                    >Senior Research Scientists</span
                                >
                            </h1>
                            <ul class="staff">
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/baroni.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_baroni.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Simona Baroni</b> Research
                                                Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/stefania.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_stefania.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Rachele Stefania</b>
                                                Research Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/alberti.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_alberti.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Diego Alberti</b> Research
                                                Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                            </ul>
                            <h1>
                                <span id="staff_title_3"
                                    >Post Doctoral Research Fellows</span
                                >
                            </h1>
                            <ul class="staff">
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/anemone.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_anemone.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Annasofia Anemone</b>
                                                Research Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/ruggiero.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_ruggiero.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Maria Rosaria Ruggiero</b>
                                                Research Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/garello.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_garello.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Francesca Garello</b>
                                                Research Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/digregorio.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_digregorio.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Enza Di Gregorio</b>
                                                Research Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/cavallari.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_cavallari.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Eleonora Cavallari</b>
                                                Research Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/capozza.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_capozza.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Martina Capozza</b> Research
                                                Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/zullino.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_zullino.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Sara Zullino</b> Research
                                                Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/romdhane.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_romdhane.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Feriel Romdhane</b> Research
                                                Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/tear.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_tear.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Louise Tear</b> Research
                                                Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                            </ul>
                            <h1>
                                <span id="staff_title_4"
                                    >PhD and Graduate Students</span
                                >
                            </h1>
                            <ul class="staff">
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/hawala.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_hawala.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Ivan Hawala</b> PhD
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/bitonto.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_bitonto.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Valeria Bitonto</b> PhD
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/tripepi.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_tripepi.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Martina Tripepi</b> PhD
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/patrucco.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_patrucco.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Deyssy Patrucco</b> PhD
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/irrera.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_irrera.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Pietro Irrera</b> PhD
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/fatehbasharzad.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_fatehbasharzad.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Parisa Fatehbasharzad</b>
                                                PhD student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/sforzi.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_sforzi.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Jacopo Sforzi</b> PhD
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/furlan.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_furlan.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Chiara Furlan</b> PhD
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/palagi.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_palagi.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Lorenzo Palagi</b> PhD
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/dhakan.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_dhakan.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Chetan Dhakan</b> PhD
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/bondar.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_bondar.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Oksana Bondar</b> PhD
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/villano.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_villano.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Daisy Villano</b> Graduate
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                            </ul>
                        </div>
                        <div class="paddingInner">
                            <div class="band">
                                <h1
                                    class="subsectionTitle"
                                    id="staff_institution_2"
                                >
                                    Institute of Biostructures and Bioimaging
                                    (CNR)
                                </h1>
                            </div>
                            <h1>
                                <span id="staff_title_5"
                                    >Research Group Leaders</span
                                >
                            </h1>
                            <ul class="staff">
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/Dario_Longo.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="PI/Longo/home.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Dario Longo</b>
                                                Researcher</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/menchise.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_menchise.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Valeria Menchise</b>
                                                Researcher</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <!-- <li class='staff' style='background-image: url(<?php echo $localizer; ?>images/staff/dandrea.webp);'>
								<div>
									<a
										title='Profile'
										href='PI/DAndrea/home.php'
										target='_blank'>
										<span style='font-size: 1.00rem;'><b>Luca Domenico D'Andrea</b>
											Researcher</span>
									</a>
								</div>
							</li> -->
                            </ul>
                            <h1>
                                <span id="staff_title_6"
                                    >Senior Research Scientists</span
                                >
                            </h1>
                            <ul class="staff">
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/padovan.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_padovan.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Sergio Padovan</b> Technical
                                                Assistant</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/carrera.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_carrera.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Carla Carrera</b> Research
                                                Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                            </ul>
                            <h1>
                                <span id="staff_title_7"
                                    >Post Doctoral, PhD and Graduate
                                    Students</span
                                >
                            </h1>
                            <ul class="staff">
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/carella.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_carella.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Antonella Carella</b>
                                                Research Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/corrado.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_corrado.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Alessia Corrado</b> Graduate
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                            </ul>
                        </div>
                        <div class="paddingInner">
                            <div class="band">
                                <h1
                                    class="subsectionTitle"
                                    id="staff_institution_3"
                                >
                                    University of Eastern Piedmont Amedeo
                                    Avogadro
                                </h1>
                            </div>
                            <ul class="staff">
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/botta.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_botta.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Mauro Botta</b> Full
                                                Professor</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/tei.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_tei.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Lorenzo Tei</b> Associate
                                                Professor</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/digilio.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_digilio.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Giuseppe Digilio</b>
                                                Assistant Professor</span
                                            >
                                        </a>
                                    </div>
                                </li>
                            </ul>
                        </div>
                        <div class="paddingInner">
                            <div class="band">
                                <h1
                                    class="subsectionTitle"
                                    id="staff_institution_4"
                                >
                                    Former Members
                                </h1>
                            </div>
                            <ul class="staff">
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/arena.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_arena.php"
                                            target="_blank"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Francesca Arena</b> Research
                                                Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/capuana.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_capuana.php"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Federico Capuana</b> PhD
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/atlante.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_atlante.php"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Sandra Atlante</b> Research
                                                Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/consolino.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_consolino.php"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Lorena Consolino</b>
                                                Research Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/pagoto.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="#"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Amerigo Pagoto</b> Research
                                                Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/paglialonga.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="#"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Alessandro Paglialonga</b>
                                                Graduate student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/spatola.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="#"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Giulia Spatola</b> PhD
                                                student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/bhavnarani.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_bhavnarani.php"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Bhavna Rani</b> Research
                                                Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/bardini.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_bardini.php"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Paola Bardini</b> Research
                                                Fellow</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/barale.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_barale.php"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Andrea Barale</b> Graduate
                                                Student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/marini.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_marini.php"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Giada Maria Marini</b>
                                                Graduate Student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                                <li
                                    class="staff"
                                    style="
                                        background-image: url(<?php echo $localizer; ?>images/staff/galati.webp);
                                    "
                                >
                                    <div>
                                        <a
                                            title="Profile"
                                            href="staff/staff_galati.php"
                                            target="_self"
                                        >
                                            <span style="font-size: 1rem"
                                                ><b>Antonello Galati</b>
                                                Graduate Student</span
                                            >
                                        </a>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div id="after"></div>
                <div id="footer">
                    <p>
                        Molecular Imaging Center - Via Nizza, 52 - I-10125
                        Torino - ITALY <br />
                        Fax. Tel. Mail
                    </p>
                    <?php include $localizer .
                    'includes/HTML5_badge_valid.inc.php';?> <?php include
                    $localizer . 'includes/PageSpeedTest.inc.php';?>
                </div>
            </div>
        </div>
        <script>
            changeSideNavStyles();
        </script>
    </body>
</html>
