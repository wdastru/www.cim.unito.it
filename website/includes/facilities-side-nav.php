<<<<<<< Updated upstream
<?php
echo "
        <div id='sidebar'>
            <div class='padding'>
                <ul id='list'>
                    <li id='section3-subsection1' class='subsection_link'><a href=
                        'mri.php'>MRI</a></li>                   
                    <li id='section3-subsection2' class='subsection_link'><a href=
                        'pet.php'>PET/SPECT/CT</a></li>
                    <li id='section3-subsection3' class='subsection_link'><a href=
                        'oi_us_pa.php'>Optical/US/PA Imaging</a></li>
                    <li id='section3-subsection4' class='subsection_link'><a href=
                        'nmr_relax.php'>NMR/Relaxometry</a></li>
                    <li id='section3-subsection5' class='subsection_link'><a href=
                        'org_lab.php'>Organic Laboratory</a></li>
                    <li id='section3-subsection6' class='subsection_link'><a href=
                        'cell_lab.php'>Cell Laboratory</a></li>
                    <li id='section3-subsection7' class='subsection_link'><a href=
                        'service.php'>Service request</a></li>
                </ul>
            </div>
        </div>
";
?>              
||||||| Stash base
<?php
echo "
        <div id='sidebar'>
            <div class='padding'>
                <dl id='list'>
                    <dt id='section3-subsection1' class='subsection_link'><a href=
                        'mri.php'>MRI</a></dt>                   
                    <dt id='section3-subsection2' class='subsection_link'><a href=
                        'pet.php'>PET/SPECT/CT</a></dt>
                    <dt id='section3-subsection3' class='subsection_link'><a href=
                        'oi_us_pa.php'>Optical/US/PA Imaging</a></dt>
                    <dt id='section3-subsection4' class='subsection_link'><a href=
                        'relax.php'>NMR/Relaxomtry</a></dt>
                    <dt id='section3-subsection5' class='subsection_link'><a href=
                        'org_lab.php'>Organic Laboratory</a></dt>
                    <dt id='section3-subsection6' class='subsection_link'><a href=
                        'cell_lab.php'>Cell Laboratory</a></dt>
                    <dt id='section3-subsection7' class='subsection_link'><a href=
                        'service.php'>Service request</a></dt>
                </dl>
            </div>
        </div>
";
?>              
=======
<?php
echo "
        <div id='sidebar'>
            <div class='padding'>
                <dl id='list'>
                    <dt id='section3-subsection1' class='subsection_link'><a href=
                        'mri.php'>MRI</a></dt>                   
                    <dt id='section3-subsection2' class='subsection_link'><a href=
                        'pet.php'>PET/SPECT/CT</a></dt>
                    <dt id='section3-subsection3' class='subsection_link'><a href=
                        'oi_us_pa.php'>Optical/US/PA Imaging</a></dt>
                    <dt id='section3-subsection4' class='subsection_link'><a href=
                        'relax.php'>NMR/Relaxomtry</a></dt>
                    <dt id='section3-subsection5' class='subsection_link'><a href=
                        'org_lab.php'>Organic Laboratory</a></dt>
                    <dt id='section3-subsection6' class='subsection_link'><a href=
                        'cell_lab.php'>Cell Laboratory</a></dt>
                    <dt id='section3-subsection7' class='subsection_link'><a href=
                        'service.php'>Service request</a></dt>
                </dl>
            </div>
        </div>
";
?>              
>>>>>>> Stashed changes
